-- Migration: Portfolio statistics backfill + duplicate index cleanup
-- Date: 2026-02-12
-- Purpose:
--   C4: Create update_portfolio_statistics() function and backfill zeroed stats
--   M7: Drop 4 duplicate/redundant indexes on ai_decisions and virtual_positions

-- ============================================
-- C4: Portfolio statistics computation function
-- ============================================
...
BEGIN
  ...
  LOOP
    ...
    -- Update portfolio with computed stats
...
-- Execute backfill
SELECT update_portfolio_statistics();

-- ============================================
-- M7: Drop duplicate/redundant indexes
-- ============================================

-- idx_ai_decisions_model_time duplicates idx_ai_decisions_model_created (both: model_id, created_at DESC)
DROP INDEX IF EXISTS idx_ai_decisions_model_time;

-- idx_ai_decisions_model_id is redundant (covered by idx_ai_decisions_model_created leading column)
DROP INDEX IF EXISTS idx_ai_decisions_model_id;

-- idx_virtual_positions_status_model_id duplicates idx_virtual_positions_status (both: status, model_id)
DROP INDEX IF EXISTS idx_virtual_positions_status_model_id;

-- idx_virtual_positions_model_id is redundant (covered by idx_virtual_positions_model_status leading column)
DROP INDEX IF EXISTS idx_virtual_positions_model_id;
